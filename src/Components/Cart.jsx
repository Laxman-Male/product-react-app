  ...
  return (
    <>
      <ul className="absolute top-2 right-2 max-h-[80vh] overflow-y-auto  ">
        <li className="cursor-pointer" onClick={handleCartDesign}>
          <lord-icon
      ...
      <li
        className={`absolute top-16 list-none right-1 rounded-full w-4 h-4 flex  justify-center items-center text-black bg-red-600  ${
          total == 0 ? "hidden" : ""
        } `}
      >
        {" "}
      ...
      <div
        className={`fixed top-12 right-1 max-h-[80vh] overflow-y-auto  border border-violet-400 w-3/4  bg-violet-300  rounded-xl ${
      ...
      >
        <div className="flex   justify-center ">
          <div className="  grid lg:grid-cols-4 xl:grid-cols-4      rounded-lg  shadow-2xl  md:grid-cols-2 sm:grid-cols-2  ">
            {cartitem.map((item) => (
              <div key={item.id}>
                <img
                  className="w-3/4 h-44  m-2 items-center shadow-xl shadow-violet-700/80  rounded-tr-3xl rounded-bl-3xl "
                  src={item.img}
                />
                ...
                <div className=" flex items-center   ml-3 ">
                  <button
                    className="border border-violet-700 font-bold w-8 rounded-l-md"
                    onClick={() => handleminus(item)}
                  >
                    {" "}
                    -{" "}
                  </button>
                  <p className="border border-violet-700 px-2  w-8 m-0 ">
                    {" "}
                    {quantity[item.id] || 1}{" "}
                  </p>
                  <button
                    className="border border-violet-700 font-bold w-8 rounded-r-md"
                ...
                <button
                  className=" border border-violet-500 rounded-xl py-1 px-4 my-1 text-lg font-semibold ml-3   "
        ...
        <div className="flex justify-center">
          <p
            className=" cursor-pointer p-2 text-blue-700 font-normal underline"
            onClick={continue_shop}
          >
            {" "}
            Continue Shopping{" "}
          </p>
